...
Train_vehs=Vehs["Train_vehs"]
Gallery_vehs=Vehs["Gallery_vehs"]
...
for veh in Gallery_vehs:
    for img in Dataset[veh].keys():
        ...
        img_out = f'{Gallery_output}/{output_name}.png'
        img = cv2.imread(img_input)
        cv2.imwrite(img_out, img)

        # yaml
        yaml_input = f'{file_path}.yaml'
        yaml_ouput = f'{Gallery_output}/{output_name}.yaml'
